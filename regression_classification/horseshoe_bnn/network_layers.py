...
class ReHorseshoeLayer(nn.Module):
    """
    Single linear layer of a horseshoe prior for regression
    """
    def __init__(self, in_features, out_features, parameters, device):
        ...
        self.log_c_rho = nn.Parameter(torch.ones([1, 1]) * parameters.log_v_rho_scale)
    ...
    def forward(self, input_, sample=True, n_samples=1):
        ...
        log_c = torch.unsqueeze(self.log_c.sample(n_samples), 1)
        log_tau_hat = (log_c * log_tau) ** 2 / (log_c**2 + log_tau ** 2 * log_v ** 2)
